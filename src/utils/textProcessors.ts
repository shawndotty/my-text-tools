...
export class TextProcessor {
	...
	processText(
		...
		switch (type) {
			case "dedupe":
				processedBody = this.processDedupe(
					lines,
					settings.dedupeIncludeEmpty
				);
				if (!hideNotice) {
				...
				break;
			case "empty-line":
				processedBody = this.processEmptyLine(
					lines,
					settings.emptyLineMode
				);
				if (!hideNotice) {
					new Notice(
			...
			case "combination-generator":
				processedBody = this.processCombinationGenerator(
	 ...
	 * 去重处理
	 */
	private processDedupe(lines: string[], includeEmpty: boolean): string {
		if (includeEmpty) {
	 ...
	 * 空行处理
	 */
	private processEmptyLine(lines: string[], mode: "all" | "merge"): string {
		if (mode === "all") {
			// 模式 1：彻底删除所有空行
	 ...
	 * 移除空白字符处理
	 */
	private processRemoveWhitespace(
		text: string,
		wsCompress: boolean,
		wsTrim: boolean,
		wsAll: boolean,
